...
################################# RELATORIO GANHOS ###############################################
def gerar_relatorio_ganhos():
    movimentacoes = dados_relatorio_ganhos()

    if not movimentacoes:
        print("Nenhum ganho encontrado para gerar relatório.")
        return    
    ...
    for mov in movimentacoes:
        ...
        pdf.cell(30, 8, data_formatada, border=1)
        pdf.cell(50, 8, origem[:30], border=1)
        pdf.set_text_color(0, 150, 0)
        pdf.cell(30, 8, valor_str, border=1)
        pdf.set_text_color(0, 0, 0)
        pdf.multi_cell(150, 8, descricao, border=1)

        pdf.set_xy(x, y + max(8, pdf.get_y() - y))
